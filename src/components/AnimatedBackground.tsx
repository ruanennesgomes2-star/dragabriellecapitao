...
export const AnimatedBackground = () => {
  ...
  useEffect(() => {
    ...
    // Render at half resolution for big performance gain
    const scale = 0.5;
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          // Reduced from 35 to 18 iterations — same visual, much lighter
          for (float i = 0.0; i < 18.0; i++) {
            v = p + cos(i * i + (iTime + p.x * 0.08) * 0.025 + i * vec2(13.0, 11.0)) * 3.5;
            float tailNoise = fbm(v + vec2(iTime * 0.3, i)) * 0.3 * (1.0 - (i / 18.0));
            // Paleta baseada em #D39A4A (211, 154, 74)
            vec4 auroraColors = vec4(
              0.827 + 0.1 * sin(i * 0.2 + iTime * 0.4),
              0.603 + 0.1 * cos(i * 0.3 + iTime * 0.5),
              0.290 + 0.1 * sin(i * 0.4 + iTime * 0.3),
              1.0
            );
    ...
    // Frame limiter: target 30fps for background animation (imperceptible diff, big perf win)
    let frameId: number;
    let lastTime = 0;
    const fps = 30;
